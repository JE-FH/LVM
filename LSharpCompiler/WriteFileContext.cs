...
{
	internal struct WriteFileContext(nint accumulator, nint accumulatorSize)
	{
		public nint _accumulator = accumulator;
		...
		public static unsafe int StreamWriter(nint L, void* dataPointer, nuint size, nint userData)
		{
			...
			if (ctxHandle.Target is WriteFileContext ctx)
			{
				...
				if (ctx._accumulatorSize < newEnd)
				{
					...
					ctx._accumulator = Marshal.ReAllocHGlobal(ctx._accumulator, newSize);
